...
export function DataTableToolbar({
  ...
  return (
    <View
      className={cn(
        'flex-row flex-wrap items-center gap-2 border-b border-gray-200 bg-gray-50/50 px-3 py-2 dark:border-gray-700 dark:bg-gray-800/30',
        className,
      )}
    >
      {filterableColumns.map((col) => (
        <View key={col.key} className="min-w-[120px] max-w-[200px]">
          <Text className="mb-0.5 text-xs text-gray-500 dark:text-gray-400">
            {col.title}
          </Text>
          <Input
