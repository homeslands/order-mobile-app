...
export const CartFooter = memo(function CartFooter({
  ...
  const checkoutBtnBg = useMemo(
    () => ({
      backgroundColor: isOrderDisabled
        ? isDark
          ? colors.gray[700]
          : colors.gray[300]
        : primaryColor,
    ...
    <>
      ...
      >
        ...
        >
          ...
          <Text
            style={[
              footerStyles.voucherLabel,
              footerStyles.voucherLabelFlex,
              ft.mutedColor,
            ]}
          >
            {t(voucher ? 'menu.changeVoucher' : 'menu.voucher', 'Mã giảm giá')}
          </Text>
          {voucher && (
            <View style={footerStyles.voucherRight}>
              {voucherDiscount > 0 && (
                <View
                  style={[footerStyles.discountBadge, ft.primaryBorderStyle]}
                >
                  <Text
                    style={[
                      footerStyles.discountBadgeText,
                      ft.primaryColorStyle,
                    ]}
                  >
        ...
        <View style={footerStyles.bottomRow}>
          ...
          >
            <ShoppingBag
              size={18}
              color={
                isOrderDisabled ? ft.disabledIconColor : colors.white.light
              }
            />
            <Text
              style={[
                footerStyles.checkoutText,
                isOrderDisabled && ft.disabledTextColor,
              ]}
            >
              {orderBtnLabel}
            </Text>
