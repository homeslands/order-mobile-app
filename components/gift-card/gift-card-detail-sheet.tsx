  ...
  return (
    <View style={[cr.row, { backgroundColor: rowBg, borderColor }]}>
      <View style={cr.left}>
        ...
        <Text style={[cr.value, { color: textColor }]} selectable>
          {value}
        </Text>
      </View>
      <Pressable
...
export const GiftCardDetailSheet = memo(function GiftCardDetailSheet({
  ...
  const handleRedeem = useCallback(() => {
    ...
    redeem(
      { serial: card.serial, code: card.code, userSlug },
      {
        onSuccess: (res) => {
          showToast(
            t('redeem.success.pointsValue', {
              points: formatPoints(res.result.cardPoints),
            }),
          )
          onClose()
        },
        onError: (err: Error) => {
          const code = (
            err as Error & { response?: { data?: { statusCode?: number } } }
  ...
  const statusCfg = useMemo(() => {
    ...
    if (card.status === GiftCardUsageStatus.AVAILABLE)
      return {
        ...
        text: isDark ? colors.success.dark : colors.success.light,
    ...
    return {
      ...
      text: isDark ? colors.destructive.dark : colors.destructive.light,
  ...
  const expiryLine = (() => {
    ...
    const days = Math.max(
      0,
      Math.floor((new Date(card.expiredAt).getTime() - nowMs) / 86_400_000),
    )
    ...
    >
      <BottomSheetScrollView
        contentContainerStyle={[
          s.content,
          { paddingBottom: isAvailable ? 16 : bottom + 24 },
        ]}
      ...
      >
        {isLoading ? (
          ...
          <>
            ...
            <View style={s.header}>
              <View
                style={[s.iconCircle, { backgroundColor: `${primaryColor}15` }]}
              >
            ...
            {card.usedAt && (
              <View
                style={[s.section, { backgroundColor: sectionBg, borderColor }]}
              >
      ...
      {isAvailable && card && (
        <View style={[s.footer, { paddingBottom: bottom + 12 }]}>
          <Pressable
            ...
            style={[
              s.redeemBtn,
              { backgroundColor: primaryColor, opacity: isRedeeming ? 0.7 : 1 },
            ]}
          >
            {isRedeeming ? (
