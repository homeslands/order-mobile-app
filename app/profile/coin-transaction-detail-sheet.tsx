...
export const CoinTransactionDetailSheet = memo(
  function CoinTransactionDetailSheet({
    visible,
    transaction,
    onClose,
  }: Props) {
    ...
    const isGiftCardIn =
      isIn && txObjType === PointTransactionObjectType.GIFT_CARD
    const isOutCardOrder =
      isOut && txObjType === PointTransactionObjectType.CARD_ORDER
      ...
      >
        <BottomSheetScrollView
          contentContainerStyle={[
            s.content,
            { paddingBottom: isOutCardOrder ? bottom + 80 : bottom + 24 },
          ]}
        ...
        >
          ...
          <View style={s.hero}>
            ...
            <Text style={[s.heroTypeLabel, { color: typeColor }]}>
              {typeLabel}
            </Text>
            <Text style={[s.heroAmount, { color: typeColor }]}>
              {cfg.prefix}
              {formatPoints(transaction.points)} {t('profile.coin.unit')}
            </Text>
          </View>

          {/* ── Desc ─────────────────────────────────────────────────── */}
          {!!transaction.desc && (
            <Text style={[s.desc, { color: textColor }]}>
              {transaction.desc}
            </Text>
          )}
          ...
          <View style={[s.card, { backgroundColor: cardBg, borderColor }]}>
            <InfoRow
              label={t('profile.coin.transactionDate')}
              subColor={subColor}
            >
            ...
            {isOut && !!transaction.objectSlug && (
              <>
                ...
                <InfoRow
                  label={t('profile.coin.orderCode')}
                  subColor={subColor}
                >
          ...
          {isGiftCardIn && (
            <View style={[s.card, { backgroundColor: cardBg, borderColor }]}>
              {isLoadingCard ? (
                ...
                <>
                  <InfoRow
                    label={t('profile.coin.giftCardName')}
                    subColor={subColor}
                  >
                    <Text
                      style={[s.rowVal, { color: textColor }]}
                      numberOfLines={2}
                    >
                      {giftCard.cardName}
                    </Text>
        ...
        {isOutCardOrder && (
          <View
            style={[
              s.footer,
              { paddingBottom: bottom + 12, borderTopColor: borderColor },
            ]}
          >
            {/* <Pressable
